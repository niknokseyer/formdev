...
function cke_create_instance($data = array()) {
    ...
    //remove unnecessary line breaks and stuff (added by scyu)
    $return .= ',on:{instanceReady:function(ev){';
    $return .= "var blockTags = ['div','h1','h2','h3','h4','h5','h6','p','pre'];";
    $return .= 'var rules = {';
    $return .= 'indent:false, breakBeforeOpen:false, breakAfterOpen:false, breakBeforeClose:false,breakAfterClose:true';
    $return .= '};';
    $return .= 'for(var i=0; i<blockTags.length; i++) {';
    $return .= 'ev.editor.dataProcessor.writer.setRules(blockTags[i],rules);';
    $return .= '}}}';
