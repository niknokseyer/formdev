...
#               a. getProfile (ProfileId) - gets the profile of the Profile with the corresponding id
#       3. STORE DATA:
#               a. newProfile () - adds a new Profile
#       4. MODIFY DATA:
#               a. setProfileValue (item, value, ProfileId) - change the value of the corresponding item of an Profile's profile 
...
#       class monitoring variables.                     
        ...
        # handles the error and notification messages of the class to notify the profile 
...
##############################                  
        public function getProfile ($ProfileId) {
                # check if the DB_CONNECTION has connected to the database
                        /*if ($this->DB_CONNECTION == NULL) {
                                ...
                                # prepare the statement
                                        $statement = "SELECT * FROM profile WHERE idprofile = '$ProfileId'";
                                # query & check if error has occured
                                        if(!$result = mysql_query($statement)) {
                                                return $errorArray['DB_QUERYERROR'] . ' -[ProfileHandle:getProfile]';
                                        }
                                        else {
                ...
                public function getProfileItem ($item, $ProfileId) {
                # check if the DB_CONNECTION has connected to the database
                        /*if ($this->DB_CONNECTION == NULL) {
                                return $errorArray['DB_NOTCONNECTED'];
                        }
                                ...
                                # prepare the statement
                                        $statement = "SELECT $item FROM profile WHERE idprofile = '$ProfileId'";
                                # query & check if error has occured
                                        if(!$result = mysql_query($statement)) {
                                                return $errorArray['DB_QUERYERROR'] . ' -[ProfileHandle:getProfileItem]';
                                        }
                                        else {
                                                # debugging purposes <remove on production>     
                                                        //echo "got it";
...
############################### 
        ...
        public function addProfile ($valuesArray) {
                # check if the DB_CONNECTION has connected to the database
                        /*if ($this->DB_CONNECTION == NULL) {
                                return $errorArray['DB_NOTCONNECTED'];
                        }
                                ...
                                # prepare the statement 
                                        $statement = "INSERT INTO profile (                              
                                                ...
                                                VALUES (
                                                        ...
                                                        '{$valuesArray['hometown']}',
                                                        '{$valuesArray['highschool']}',
                                                        '{$valuesArray['verse']}'
                                                )";
                                        
                                # query & check if error has occured
                                        if(!$result = mysql_query($statement)) {
                                                return $errorArray['DB_QUERYERROR'] . ' -[ProfileHandle:addProfile]';
                                        }
                                        else {
                                                # debugging purposes <remove on production>     
...
#########################
        ...
        public function setProfileValue ($item, $value, $ProfileId) {
                # check if the DB_CONNECTION has connected to the database
                        /*if ($this->DB_CONNECTION == NULL) {
                                return $errorArray['DB_NOTCONNECTED'];
                        }
                                ...
                                $ProfileId = addslashes($ProfileId);
                                                # prepare the statement
                                                        $statement = "UPDATE profile SET $item = '$value' WHERE idprofile = '$ProfileId'";  
                                                # query & check if error has occured
                                                        if(!$result = mysql_query($statement)) {
                                                                return $errorArray['DB_QUERYERROR'] . ' -[ProfileHandle:setProfileValue]';
                                                        }
                                                        else {
                                                                # debugging purposes <remove on production>     
                                                                        //echo "updated the database";
