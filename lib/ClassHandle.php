...
#               a. getClass (ClassId) - gets the class of the Class with the corresponding id
#       3. STORE DATA:
#               a. newClass () - adds a new Class
#       4. MODIFY DATA:
#               a. setClassValue (item, value, ClassId) - change the value of the corresponding item of an Class's class 
...
#       class monitoring variables.                     
        ...
        # handles the error and notification messages of the class to notify the class 
...
##############################                  
        public function getClass ($ClassId) {
                # check if the DB_CONNECTION has connected to the database
                        /*if ($this->DB_CONNECTION == NULL) {
                                ...
                                # prepare the statement
                                        $statement = "SELECT * FROM class WHERE idclass = '$ClassId'";
                                # query & check if error has occured
                                        if(!$result = mysql_query($statement)) {
                                                return $errorArray['DB_QUERYERROR'] . ' -[ClassHandle:getClass]';
                                        }
                                        else {
                ...
                public function getClassItem ($item, $ClassId) {
                # check if the DB_CONNECTION has connected to the database
                        /*if ($this->DB_CONNECTION == NULL) {
                                return $errorArray['DB_NOTCONNECTED'];
                        }
                                ...
                                # prepare the statement
                                        $statement = "SELECT $item FROM class WHERE idclass = '$ClassId'";
                                # query & check if error has occured
                                        if(!$result = mysql_query($statement)) {
                                                return $errorArray['DB_QUERYERROR'] . ' -[ClassHandle:getClassItem]';
                                        }
                                        else {
                                                # debugging purposes <remove on production>     
                                                        //echo "got it";
...
############################### 
        ...
        public function addClass ($valuesArray) {
                # check if the DB_CONNECTION has connected to the database
                        /*if ($this->DB_CONNECTION == NULL) {
                                return $errorArray['DB_NOTCONNECTED'];
                        }
                                ...
                                # query & check if error has occured
                                        if(!$result = mysql_query($statement)) {
                                                return $errorArray['DB_QUERYERROR'] . ' -[ClassHandle:addClass]';
                                        }
                                        else {
                                                # debugging purposes <remove on production>     
...
#########################
        ...
        public function setClassValue ($item, $value, $ClassId) {
                # check if the DB_CONNECTION has connected to the database
                        /*if ($this->DB_CONNECTION == NULL) {
                                return $errorArray['DB_NOTCONNECTED'];
                        }
                                ...
                                $ClassId = addslashes($ClassId);
                                                # prepare the statement
                                                        $statement = "UPDATE class SET $item = '$value' WHERE idclass = '$ClassId'";  
                                                # query & check if error has occured
                                                        if(!$result = mysql_query($statement)) {
                                                                return $errorArray['DB_QUERYERROR'] . ' -[ClassHandle:setClassValue]';
                                                        }
                                                        else {
                                                                # debugging purposes <remove on production>     
                                                                        //echo "updated the database";
