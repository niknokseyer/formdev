...
#               a. getWorkbook (WorkbookId) - gets the workbook of the Workbook with the corresponding id
#       3. STORE DATA:
#               a. newWorkbook () - adds a new Workbook
#       4. MODIFY DATA:
#               a. setWorkbookValue (item, value, WorkbookId) - change the value of the corresponding item of an Workbook's workbook 
...
#       class monitoring variables.                     
        ...
        # handles the error and notification messages of the class to notify the workbook 
...
########################################
        public function WorkbookHandle(){
...
##############################                  
        public function getWorkbook ($WorkbookId) {
                # check if the DB_CONNECTION has connected to the database
                        /*if ($this->DB_CONNECTION == NULL) {
                                ...
                                # protection against SQL injection, add slashes to escape all escape characters
                                        $WorkbookId = addslashes($WorkbookId);
                                # prepare the statement
                                        $statement = "SELECT * FROM workbook WHERE idworkbook = '$WorkbookId'";
                                # query & check if error has occured
                                        if(!$result = mysql_query($statement)) {
                                                return $errorArray['DB_QUERYERROR'] . ' -[WorkbookHandle:getWorkbook]';
                                        }
                                        else {
                ...
                public function getWorkbookItem ($item, $WorkbookId) {
                # check if the DB_CONNECTION has connected to the database
                        /*if ($this->DB_CONNECTION == NULL) {
                                return $errorArray['DB_NOTCONNECTED'];
                        }
                                ...
                                # prepare the statement
                                        $statement = "SELECT $item FROM workbook WHERE idworkbook = '$WorkbookId'";
                                # query & check if error has occured
                                        if(!$result = mysql_query($statement)) {
                                                return $errorArray['DB_QUERYERROR'] . ' -[WorkbookHandle:getWorkbookItem]';
                                        }
                                        else {
                                                # debugging purposes <remove on production>     
                                                        //echo "got it";
...
############################### 
        ...
        public function addWorkbook ($valuesArray) {
                # check if the DB_CONNECTION has connected to the database
                        /*if ($this->DB_CONNECTION == NULL) {
                                return $errorArray['DB_NOTCONNECTED'];
                        }
                                ...
                                # prepare the statement 
                                        $statement = "INSERT INTO workbook (                              
                                                        idstudent,
                                                        ch1,
                                                        ch2,
                                                        ch3,
                                ...
                                # query & check if error has occured
                                        if(!$result = mysql_query($statement)) {
                                                return $errorArray['DB_QUERYERROR'] . ' -[WorkbookHandle:addWorkbook]';
                                        }
                                        else {
                                                # debugging purposes <remove on production>     
...
#########################
        ...
        public function setWorkbookValue ($item, $value, $WorkbookId) {
                # check if the DB_CONNECTION has connected to the database
                        /*if ($this->DB_CONNECTION == NULL) {
                                return $errorArray['DB_NOTCONNECTED'];
                        }
                                ...
                                $WorkbookId = addslashes($WorkbookId);
                                                # prepare the statement
                                                        $statement = "UPDATE workbook SET $item = '$value' WHERE idworkbook = '$WorkbookId'";  
                                                # query & check if error has occured
                                                        if(!$result = mysql_query($statement)) {
                                                                return $errorArray['DB_QUERYERROR'] . ' -[WorkbookHandle:setWorkbookValue]';
                                                        }
                                                        else {
                                                                # debugging purposes <remove on production>     
                                                                        //echo "updated the database";
